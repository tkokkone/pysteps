...
while n_sim < n_simulations:
    ...
    azero_files = os.listdir(azero_dir)
    for file in azero_files:
        if file.startswith("azero"):
            azero_file = file
    azero_autocor = genfromtxt(fname=os.path.join(azero_dir, azero_file), delimiter=',', skip_header=1)
    azero_autocor = np.delete(azero_autocor, 0, axis=1)
    azero_opt = int(azero_autocor[1,:][azero_autocor[2,:] == np.min(azero_autocor[2,:])])
    ...
    # np.mean(r_mean_scaled) = 8.35552542273175
    # mu_z = 8.35552542273175
    r_mean = r_mean_scaled.copy()
    
    plt.figure()
    plt.plot(r_mean_org, label="bl_org")
    plt.plot(r_mean, label="bl_scaled") 
    plt.plot(data_tss[0,:], label="osapol", linestyle="--", color="Red")
    plt.legend()
    plt.title("Mean")
    ...
    if show_cumul_plot:
        ...
        plt.colorbar()
        if save_cumul_plot:
            plt.savefig(os.path.join(out_dir, "accumulation_plot.png"))
            ...
            acc_plot = rasterio.open(os.path.join(out_dir, "accumulation_raster.tif"), "w", driver="GTiff",
    ...
    if csv_brokenlines:
        data_temp2 = [r_mean[:,0], r_mean_org[:,0], v_mag[:,0], v_dir[:,0], vx[:,0], vy[:,0]]
        bl_arrays_to_save = pd.DataFrame(data_temp2, index=['r_mean_scaled', 'r_mean_org', 'v_mag', 'v_dir', 'vx', 'vy'])
