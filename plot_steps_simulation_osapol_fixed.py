...
all_params = genfromtxt(fname="//home.org.aalto.fi/lindgrv1/data/Desktop/Vaitoskirjaprojekti/Tutkimus/parameters_log/db/all_params.csv",
                        delimiter=',', skip_header=1, usecols=1)

# Read in time fft parameters
fft_params = genfromtxt(
    fname="//home.org.aalto.fi/lindgrv1/data/Desktop/Vaitoskirjaprojekti/Tutkimus/parameters_log/db/fft_params.csv", delimiter=',', skip_header=1)
fft_params = np.delete(fft_params, 0, axis=1)

# Read in time series from input data
data_tss = genfromtxt(
    fname="//home.org.aalto.fi/lindgrv1/data/Desktop/Vaitoskirjaprojekti/Tutkimus/parameters_log/db/data_tss.csv", delimiter=',', skip_header=1)
...
seed1 = 1234  # seed number for generation of the first precipitation field
...
b_v = all_params[13]
c_v = all_params[14]
a_war = 0.3214#all_params[15]
b_war = 0.01506#all_params[16]
c_war = 0.002001#all_params[17]
...
# Broken line parameters for field mean
# mean of mean areal reflectivity over the simulation period
mu_z = all_params[18]
# variance of mean areal reflectivity over the simulation period
sigma2_z = all_params[19]
...
no_bls = 1  # number of broken lines
# acceptable tolerance for variance as ratio of input variance [-]
var_tol_z = all_params[32]
# acceptable value for first and last elements of the final broken line as ratio of input mean:
mar_tol_z = all_params[33]

# Broken line parameters for velocity magnitude
# mean of mean areal reflectivity over the simulation period
mu_vmag = all_params[22]
# variance of mean areal reflectivity over the simulation period
sigma2_vmag = all_params[23]
h_val_vmag = all_params[24]  # structure function exponent
# scale ratio between levels n and n+1 (constant) [-]
q_val_vmag = all_params[30]
a_zero_vmag = all_params[25]  # time series decorrelation time [min]
no_bls = 1  # number of broken lines
# acceptable tolerance for variance as ratio of input variance [-]
var_tol_vmag = all_params[32]
# acceptable value for first and last elements of the final broken line as ratio of input mean:
mar_tol_vmag = all_params[33]

# Broken line parameters for velocity direction
# mean of mean areal reflectivity over the simulation period
mu_vdir = all_params[26]
# variance of mean areal reflectivity over the simulation period
sigma2_vdir = all_params[27]
h_val_vdir = all_params[28]  # structure function exponent
# scale ratio between levels n and n+1 (constant) [-]
q_val_vdir = all_params[30]
a_zero_vdir = all_params[29]  # time series decorrelation time [min]
no_bls = 1  # number of broken lines
# acceptable tolerance for variance as ratio of input variance [-]
var_tol_vdir = all_params[32]
# acceptable value for first and last elements of the final broken line as ratio of input mean:
mar_tol_vdir = all_params[33]
...
###############################################################################
# Create the first two precipitation fields, the number is determined by the order of
...
noise_kwargs = dict()
p_pow[0] = np.log(nx_field/(a_w0 + b_w0 * r_mean[0] +
                  c_w0 * r_mean[0] ** 2))  # scale break
...
for i in range(1, n_timesteps):
    ...
    p_pow[0] = np.log(nx_field/(a_w0 + b_w0 * r_mean[i] +
                      c_w0 * r_mean[i] ** 2))  # scale break
    p_pow[2] = (a_1 + b_1 * r_mean[i] + c_1 * r_mean[i] ** 2) # beta1
    p_pow[3] = (a_2 + b_2 * r_mean[i] + c_2 * r_mean[i] ** 2)  # beta2

    # #Just for testing beta behavior
    # if i == 5:
    #     p_pow[2] = -1.4
    #     p_pow[3] = -3
    ...
    R_new = nowcast_method(
        ...
        n_cascade_levels=6,
        R_thr=-10.0, 
        #pitäiskö tän olla eri? 8.1830486304816077?
    ...
    # were changed to -135 and 135 from 0 and 50.
...
# # Test what values comes out and why WAR is always 1?
# r_mean[50]
# a_v + b_v * r_mean[50] + c_v * r_mean[50] ** 2
# a_war + b_war * r_mean[50] + c_war * r_mean[50] ** 2

# Plot timeseries of mean
# time series of arithmetic means, ignoring NaNs
...
for i in range(0, len(R_sim)):
    ...
    #r_war[i] = 0.3214 + 0.01506 * r_mean[i] + 0.002001 * r_mean[i] ** 2
ts_war = np.zeros(len(R_sim))
for i in range(0, len(R_sim)):
    ts_war[i] = (np.count_nonzero(R_sim[i][R_sim[i] > 3.1830486304816077])) / \
        np.count_nonzero(~np.isnan(R_sim[i]))  #poista nimittäjästä nan solut
