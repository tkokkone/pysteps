...
#Wetted area ratio
#WAR have to be calculated before adding 1 to each cell of R
...
# R, metadata = transformation.dB_transform(R, metadata, threshold=0.1, zerovalue=0)
...
#Plot event as a animation
# plt.figure()
# pysteps.visualization.animations.animate(R, savefig=False, path_outputs="//home.org.aalto.fi/lindgrv1/data/Desktop/Vaitoskirjaprojekti/Tutkimus/parameters_log/db/fields_data")

##############################################################################
# # TEST: Load simulated event from txt-file
...
#Seed's C++ way to calculate spatial scale L_k
#L_k have to be calculated already here because n_cascade_levels will possibly change
...
L_k[0] = max(nx_field,ny_field) * kmperpixel
for i in range(n_cascade_levels-1):
    L_k[i+1] = L_k[i] * scale_ratio 
...
# pd.DataFrame(fft_params).to_csv("//home.org.aalto.fi/lindgrv1/data/Desktop/Vaitoskirjaprojekti/Tutkimus/parameters_log/db/data_tss.csv")
...
# pd.DataFrame(fft_params).to_csv("//home.org.aalto.fi/lindgrv1/data/Desktop/Vaitoskirjaprojekti/Tutkimus/parameters_log/db/fft_params.csv")
...
pd.DataFrame(all_params).to_csv("//home.org.aalto.fi/lindgrv1/data/Desktop/Vaitoskirjaprojekti/Tutkimus/parameters_log/db/all_params.csv")
