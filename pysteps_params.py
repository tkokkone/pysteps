...
fns = pysteps.io.archive.find_by_date(date, root_path, path_fmt, fn_pattern, fn_ext, timestep=5, num_prev_files=2)
...
#TEEMU: test r to dbz
metadata["unit"] = "mm/h"
Z, metadata = utils.conversion.to_reflectivity(R, metadata)

#Log-transform the data
R, metadata = transformation.dB_transform(R, metadata, threshold=0.15, zerovalue=-15.0)

#Change zerovalue to NaN
# R[R==-15] = NaN
...
for i in range (0, len(fns[1])):
    #print(i)
    war_ts[i] = (np.count_nonzero(R[i][R[i] > 0])) / (264*264)
# plt.figure()
# plt.plot(war_ts)
...
a_zero = 16*tStep #time steps in which correlation values are > 1/e = 0.36787968862663156
#For now, this have to be checked from plots above and insert the value manually
...
slope, intercept, r_value, p_value, std_err = sp.linregress(np.log(freqs_sliced[0:67]), np.log(power_spectrum_sliced[0:67]))
y_values = slope*np.log(freqs_sliced[0:67])+intercept

plt.figure()
plt.plot(np.log(freqs_sliced), np.log(power_spectrum_sliced))
plt.plot(np.log(freqs_sliced[0:67]), y_values)
...
a_zero_Vxy = 12*tStep #time steps in which correlation values are > 1/e = 0.36787968862663156
#For now, this have to be checked from plots above and insert the value manually
...
#Calculate power spectrum exponent beta and H from beta using linear fitting of the data
slope_Vxy, intercept_Vxy, r_value_Vxy, p_value_Vxy, std_err_Vxy = sp.linregress(np.log(freqs_Vxy_sliced[0:17]), np.log(power_spectrum_Vxy_sliced[0:17]))
y_values_Vxy = slope_Vxy*np.log(freqs_Vxy_sliced[0:17])+intercept_Vxy

plt.figure()
plt.plot(np.log(freqs_Vxy_sliced), np.log(power_spectrum_Vxy_sliced))
plt.plot(np.log(freqs_Vxy_sliced[0:17]), y_values_Vxy)
...
a_zero_Vdir_deg_adj = 8*tStep #time steps in which correlation values are > 1/e = 0.36787968862663156
#For now, this have to be checked from plots above and insert the value manually
...
#Calculate power spectrum exponent beta and H from beta using linear fitting of the data
slope_Vdir_deg_adj, intercept_Vdir_deg_adj, r_value_Vdir_deg_adj, p_value_Vdir_deg_adj, std_err_Vdir_deg_adj = sp.linregress(np.log(freqs_Vdir_deg_adj_sliced[0:20]), np.log(power_spectrum_Vdir_deg_adj_sliced[0:20]))
y_values_Vdir_deg_adj = slope_Vdir_deg_adj*np.log(freqs_Vdir_deg_adj_sliced[0:20])+intercept_Vdir_deg_adj

plt.figure()
plt.plot(np.log(freqs_Vdir_deg_adj_sliced), np.log(power_spectrum_Vdir_deg_adj_sliced))
plt.plot(np.log(freqs_Vdir_deg_adj_sliced[0:20]), y_values_Vdir_deg_adj)
...
for i in range(len(gamma2_mean), len(gamma_all)):
    gamma2_mean[i-7] = np.mean(abs(gamma_all[i,:]))
...
slope_tlen, intercept_tlen, r_value_tlen, p_value_tlen, std_err_tlen = sp.linregress(np.log(L_k[0:6,0]), np.log(tau_k[0:6,0]))
y_values_tlen = slope_tlen*np.log(L_k[0:6])+intercept_tlen

plt.figure()
plt.plot(np.log(L_k[0:7,0]), np.log(tau_k[0:7,0]), marker='s')
plt.plot(np.log(L_k[0:6]), y_values_tlen)

tlen_b = slope_tlen
tlen_a = np.exp(intercept_tlen)

#Estimate tlen_a and tlen_b based on linear fit of tau_k and L_k
slope_tlen_c, intercept_tlen_c, r_value_tlen_c, p_value_tlen_c, std_err_tlen_c = sp.linregress(np.log(gamma1_mean[0:6,0]), np.log(gamma2_mean[0:6,0]))
y_values_tlen_c = slope_tlen_c*np.log(gamma1_mean[0:6,0])+intercept_tlen_c

plt.figure()
plt.plot(np.log(gamma1_mean[0:7,0]), np.log(gamma2_mean[0:7,0]), marker='s')
plt.plot(np.log(gamma1_mean[0:6]), y_values_tlen_c)
